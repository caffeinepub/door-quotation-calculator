...
/**
 * Find the catalogue size for a DOUBLE DOOR using the special DD rule:
 *
 * STEP 1: Height — select next higher catalogue height ≥ actual height (same as normal).
 * STEP 2: Width — first find the normal rounded width (≥ actual width),
 *         then move ONE STEP HIGHER in the sorted catalogue widths for that height.
 *         If stepping one higher exceeds the available widths, use the highest available.
 *
 * Example: actual 72 × 30.375 → normal width = 32 → DD width = 34 → result: 72 × 34
 * Example: actual 78 × 31    → normal width = 32 → DD width = 34 → result: 78 × 34
 */
export function findDoubleDoorCatalogueSize(
  actualHeightInches: number,
  actualWidthInches: number
): { height: number; width: number; wasRounded: boolean } | null {
  const sortedEntries = [...CATALOGUE].sort((a, b) => a.height - b.height);
  ...
  for (const entry of sortedEntries) {
    ...
    // Find the normal rounded width index (first width >= actual)
    const normalIdx = sortedWidths.findIndex(w => w >= actualWidthInches);
    if (normalIdx === -1) {
      // No width in this height satisfies — try next height
      continue;
    }

    // Move ONE STEP HIGHER — if at the last index, stay at the last (cap at max)
    const ddIdx = Math.min(normalIdx + 1, sortedWidths.length - 1);
    const ddWidth = sortedWidths[ddIdx];

    return {
      height: entry.height,
      width: ddWidth,
      wasRounded: entry.height !== actualHeightInches || ddWidth !== actualWidthInches,
    };
  }
